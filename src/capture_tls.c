...
int P_hash(const char *digest,
    unsigned char *dest, int dlen,
    unsigned char *secret, int sslen,
...
int PRF(unsigned char *dest, int dlen,
    unsigned char *pre_master_secret, int plen,
    unsigned char *label,
    unsigned char *seed, int slen)
{
    int i;
...
tls_connection_create(struct in_addr caddr, u_short cport, struct in_addr saddr, u_short sport)
{
    ...
    SSL_CTX_use_PrivateKey_file(conn->ssl_ctx, get_option_value("capture.keyfile"), SSL_FILETYPE_PEM);
    ...
    if (UINT24_INT(handshake->length) > 0 ) {
        ...
        switch(handshake->type) {
            ...
            case client_key_exchange:
                ...
                EVP_CipherInit(&conn->client_cipher_ctx, ciph, conn->key_material.client_write_key, conn->key_material.client_write_IV, 0);
                EVP_CIPHER_CTX_init(&conn->server_cipher_ctx);
                EVP_CipherInit(&conn->server_cipher_ctx, ciph, conn->key_material.server_write_key, conn->key_material.server_write_IV, 0);
