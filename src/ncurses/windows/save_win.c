...
G_DEFINE_TYPE(SaveWindow, save, NCURSES_TYPE_WINDOW)
...
save_draw(Window *window)
{
    ...
    SaveWindow *self = NCURSES_SAVE(window);
...
{
    SaveWindow *self = NCURSES_SAVE(object);
...
save_init(SaveWindow *self)
{
    ...
    window_set_window_type(NCURSES_WINDOW(self), WINDOW_SAVE);
}
