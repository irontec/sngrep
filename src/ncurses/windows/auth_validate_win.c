...
G_DEFINE_TYPE(AuthValidateWindow, auth_validate_win, NCURSES_TYPE_WINDOW)
...
{
    AuthValidateWindow *self = NCURSES_AUTH_VALIDATE(object);
...
auth_validate_win_init(AuthValidateWindow *self)
{
    ...
    window_set_window_type(NCURSES_WINDOW(self), WINDOW_AUTH_VALIDATE);
}
